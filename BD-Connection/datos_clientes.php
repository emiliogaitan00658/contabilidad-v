...
//        $fecha = date("Y/n/j");
    ...
    */
    ...
    public static function suma_stock_materiales($indproducto, $mysqli)
    {
        $result = $mysqli->query("SELECT SUM(cantidad) as cantidad FROM `baja_materiales` WHERE codigo_producto like '$indproducto'");
        $row = $result->fetch_array(MYSQLI_ASSOC);
        if (!empty($row)) {
            return $row["cantidad"];
        }
...
VALUES (NULL, '$indsucursal', '$indcliente', NULL, '$monto', '$cuotas', '$fecha', '1', '0', '$key');";
 ...
 public static function credito_ind_tempora_cambio($key,$Confirmacion,$mysqli)
    {
        $insert = "UPDATE `credito` SET `producto` = '$Confirmacion' WHERE `credito`.`indtemp` = '$key'";
        $query = mysqli_query($mysqli, $insert);
        return true;
    }
...
//echo var_dump(strpos("$temp", 'RAX'));
    ...
    public static function buscar_producto_codigo_producto_barra_codigo($indproducto, $mysqli)
    {
        $result = $mysqli->query("SELECT * FROM `producto` WHERE fecha_vencimiento='$indproducto'");
        $row3 = $result->fetch_array(MYSQLI_ASSOC);
        if (!empty($row3)) {
            return $row3["codigo_producto"];
        }
        return "";
    }
...
VALUES (NULL, NULL, '$indproducto', '$producto','1','$precio_cordobas','$precio_cordobas', '$dolar', '0', '0' , '$fecha','$indcliente', '$indsucursal', '0', '$indtemp');";
        $query = mysqli_query($mysqli, $insert);
        return true;
    }
...
(NULL, '$RAX', '$nombre_completo', '$sucursal', '$Key', '', '$fecha', '$hora');";
    ...
    public static function nuevo_credito_itm_manual($indcliente, $indsucursal, $monto, $indfactura, $mysqli)
    {
       $Key = self::Verificar_generador_codigo($mysqli);

        $fecha = self::fecha_get_pc_MYSQL();
        $insert3 = "INSERT INTO `credito` (`indcredito`, `indsucursal`, `indcliente`, `producto`, `totalCredito`, `numeroCuotas`, `fechaInicio`, `status`, `prima`, `indtemp`) 
        VALUES (NULL, '$indsucursal', '$indcliente', '$indfactura', '$monto', '0', '$fecha', '1', '0', '$Key');";
        $query = mysqli_query($mysqli, $insert3);
        return true;
    }
...
(NULL, NULL, '$sucursal', '$Key', '$detalle', '$fecha', '$hora', '0');";
    ...
    public static function agregar_dato_producto($codigo, $producto, $precio1, $precio2, $precio3,$codigobarra, $mysqli)
    {
...
VALUES (NULL, '$codigo', '$producto', '$precio1', '$precio2', '$precio3', '$codigobarra', '1');";
        $query = mysqli_query($mysqli, $insert);
        return true;
    }
...
VALUES (NULL, 'acceso', '127.0.0.1', '$fecha', '$hora', '1', '1');";
    ...
    public static function cambio_dato_producto($indproducto, $producto, $precio1, $precio2, $precio3, $codigobarra,$mysqli)
    {
        $hora = self::hora_get_pc();
        $fecha = self::fecha_get_pc_MYSQL();
        $insert = "UPDATE `producto` SET `nombre_producto` = '$producto', `precio1` = '$precio1', `precio2` = '$precio2', `precio3` = '$precio3',`fecha_vencimiento` = '$codigobarra'
WHERE `producto`.`indproducto` = '$indproducto'";
    ...
    public static function registro_Stock($codigo,$marca,$detalle,$cantidad,$fecha,$mysqli)
    {

        $insert = "INSERT INTO `inventario_producto` (`ind_stock_producto`, `codigo_producto`, `marca`, `nombre_producto`, `cantidad`, `fecha_vencimiento`, `bandera`, `temp`)
                VALUES (NULL, '$codigo', '$marca', '$detalle', '$cantidad', '$fecha', '1', '1')";
        $query = mysqli_query($mysqli, $insert);
        return true;
    }

    public static function registro_Stock_baja($codigo,$cantidad,$fecha,$mysqli)
    {
        $hora=self::hora_get_pc();
        $insert = "INSERT INTO `baja_materiales` (`indmaterial`, `codigo_producto`, `cantidad`, `fecha`, `hora`, `bandera`, `temp`) VALUES
                                            (NULL, '$codigo', '$cantidad', '$fecha', '$hora', '1', '1');";
        $query = mysqli_query($mysqli, $insert);
        return true;
    }
